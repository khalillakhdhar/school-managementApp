        ...
        <div style="display:flex;align-items:center;gap:14px;">
            ...
            <div>
                ...
                <div style="color:rgba(255,255,255,0.6);font-size:13px;margin-top:3px;">
                    {{ ucfirst(now()->locale(app()->getLocale())->isoFormat('dddd D MMMM YYYY')) }}
                </div>
            </div>
        </div>

        <div style="text-align:end;flex-shrink:0;">
            <div style="color:rgba(255,255,255,0.6);font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;">{{ __('Année scolaire') }}</div>
            ...
            <div style="display:inline-flex;align-items:center;gap:5px;margin-top:6px;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);border-radius:20px;padding:3px 12px;">
                ...
                <span style="color:rgba(255,255,255,0.85);font-size:11px;font-weight:600;">{{ __('Système actif') }}</span>
            </div>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;position:relative;">
        ...
        <div style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);border-radius:12px;padding:18px 20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <div style="color:rgba(255,255,255,0.65);font-size:11px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;">{{ __('Élèves actifs') }}</div>
            ...
            <div style="color:rgba(255,255,255,0.5);font-size:11px;margin-top:5px;">{{ __('/ :total inscrits', ['total' => $totalStudents]) }}</div>
        </div>

        {{-- Recettes du mois --}}
        <div style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);border-radius:12px;padding:18px 20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <div style="color:rgba(255,255,255,0.65);font-size:11px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;">{{ __('Recettes du mois') }}</div>
            ...
            <div style="color:rgba(255,255,255,0.5);font-size:11px;margin-top:5px;">{{ __('TND encaissés') }}</div>
        </div>
        ...
        <div style="background:{{ $overdueCount > 0 ? 'rgba(248,113,113,0.15)' : 'rgba(255,255,255,0.1)' }};border:1px solid {{ $overdueCount > 0 ? 'rgba(248,113,113,0.3)' : 'rgba(255,255,255,0.15)' }};border-radius:12px;padding:18px 20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <div style="color:rgba(255,255,255,0.65);font-size:11px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;">{{ __('Overdue Payments') }}</div>
            ...
            <div style="color:rgba(255,255,255,0.5);font-size:11px;margin-top:5px;">{{ $overdueCount === 0 ? __('Aucun impayé') : __('paiement(s) échu(s)') }}</div>
        </div>
        ...
        <div style="background:{{ $pendingIncidents > 0 ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.1)' }};border:1px solid {{ $pendingIncidents > 0 ? 'rgba(251,191,36,0.3)' : 'rgba(255,255,255,0.15)' }};border-radius:12px;padding:18px 20px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <div style="color:rgba(255,255,255,0.65);font-size:11px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;">{{ __('Incidents en attente') }}</div>

...
<div style="display:flex;flex-direction:column;gap:16px;">
    ...
    <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:14px 20px;box-shadow:0 1px 3px rgba(16,24,40,.05);display:flex;align-items:center;gap:14px;">
        <label style="font-size:13px;font-weight:600;color:#64748b;">Enfant :</label>
        ...
        @if($className)<span style="font-size:13px;color:#94a3b8;">Classe {{ $className }}</span>@endif
    </div>

    @if($empty)
        <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:48px;text-align:center;">
            <div style="font-size:38px;">🗓️</div>
            <div style="font-size:15px;font-weight:700;color:#1e293b;margin-top:10px;">Emploi du temps non disponible</div>
            <div style="font-size:13px;color:#94a3b8;margin-top:4px;">L'emploi du temps de la classe n'a pas encore été planifié.</div>
        </div>
    @else
        ...
        <table style="width:100%;border-collapse:collapse;min-width:680px;">
            <thead>
                <tr style="background:#fafbfc;">
                    <th style="padding:12px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;text-align:left;border-bottom:1px solid #eaeef3;">Jour</th>
                    @foreach($slots as $s)
                    <th style="padding:12px 10px;font-size:11px;font-weight:700;color:#64748b;text-align:center;border-bottom:1px solid #eaeef3;border-left:1px solid #f1f4f8;">{{ $s['start'] }}<br><span style="font-weight:500;color:#94a3b8;">{{ $s['end'] }}</span></th>
                    @endforeach
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td style="padding:7px;border-bottom:1px solid #f1f4f8;border-left:1px solid #f1f4f8;vertical-align:top;">
                        @if($cell)
                        <div style="background:#eff6ff;border-left:3px solid #2563eb;border-radius:8px;padding:7px 9px;">

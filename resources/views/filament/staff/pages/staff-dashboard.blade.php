...
<div style="display:flex;flex-direction:column;gap:18px;">
    ...
    <div style="border-radius:16px;padding:24px 28px;color:#fff;background:linear-gradient(135deg,#2563eb,#1d4ed8);box-shadow:0 8px 24px rgba(37,99,235,.3);">
        <div style="font-size:13px;opacity:.85;font-weight:600;">{{ $employee->is_teacher ? 'Enseignant(e)' : $employee->position }}</div>
        <div style="font-size:24px;font-weight:800;letter-spacing:-.4px;margin-top:2px;">Bonjour, {{ $employee->first_name }} 👋</div>
        <div style="font-size:13.5px;opacity:.9;margin-top:4px;">{{ now()->locale('fr')->isoFormat('dddd D MMMM YYYY') }}</div>
    </div>

    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;">
        @foreach([
            ['Séances / semaine', $stats['sessions'], '#2563eb'],
            ['Heures / semaine', $stats['hours'].'h', '#10b981'],
            ['Mes classes', $stats['classes'], '#8b5cf6'],
            ['Mes matières', $stats['subjects'], '#f59e0b'],
        ] as $card)
        ...
        <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:20px 24px;box-shadow:0 1px 3px rgba(16,24,40,.05);">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9;">
                ...
                <span style="display:flex;align-items:center;gap:10px;">
                    ...
                    <span style="font-size:11px;font-weight:700;padding:2px 9px;border-radius:7px;background:{{ $p['status']==='paid' ? '#ecfdf5' : '#fffbeb' }};color:{{ $p['status']==='paid' ? '#059669' : '#b45309' }};">{{ $p['status']==='paid' ? 'Payée' : 'En attente' }}</span>
                </span>
            </div>
            @empty

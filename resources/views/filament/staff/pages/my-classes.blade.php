...
@if($view === 'detail')
    ...
    <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;overflow:hidden;box-shadow:0 1px 3px rgba(16,24,40,.05);">
        <div style="padding:18px 22px;border-bottom:1px solid #f1f4f8;display:flex;align-items:center;justify-content:space-between;">
            <div>
                <div style="font-size:17px;font-weight:800;color:#0f172a;">Classe {{ $class->name }}</div>
                <div style="font-size:13px;color:#64748b;">{{ $class->level?->name }} · {{ $students->count() }} élève(s)</div>
            </div>
        </div>
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="background:#fafbfc;">
                    <th style="text-align:left;padding:12px 22px;font-size:11px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:.5px;">Élève</th>
                    <th style="text-align:left;padding:12px 14px;font-size:11px;font-weight:600;color:#64748b;text-transform:uppercase;">N°</th>
                    <th style="text-align:center;padding:12px 14px;font-size:11px;font-weight:600;color:#64748b;text-transform:uppercase;">Statut</th>
                    <th style="text-align:right;padding:12px 22px;font-size:11px;font-weight:600;color:#64748b;text-transform:uppercase;">Présence (mois)</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f4f8;">
                    ...
                    <td style="padding:13px 14px;text-align:center;">
                        <span style="font-size:11px;font-weight:700;padding:2px 9px;border-radius:7px;background:{{ $s['status']==='active' ? '#ecfdf5':'#f1f5f9' }};color:{{ $s['status']==='active' ? '#059669':'#64748b' }};">{{ $s['status']==='active' ? 'Actif':'Inactif' }}</span>
                    </td>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(290px,1fr));gap:18px;">
        ...
        <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:20px;box-shadow:0 1px 3px rgba(16,24,40,.05);">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:14px;">
                ...
                @if($c['titulaire'])
                <span style="font-size:10.5px;font-weight:700;padding:3px 9px;border-radius:7px;background:#eff6ff;color:#2563eb;">TITULAIRE</span>
                @endif
            </div>
            <div style="display:flex;gap:18px;margin-bottom:14px;">
                <div><div style="font-size:20px;font-weight:800;color:#0f172a;">{{ $c['students'] }}</div><div style="font-size:11px;color:#94a3b8;font-weight:600;">ÉLÈVES</div></div>
            </div>
            @if($c['subjects'])

...
@if($classes->isEmpty())
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:14px;padding:24px;color:#92400e;">
        ...
        <div style="margin-top:6px;font-size:13px;">{{ __("Vous devez être titulaire d'une classe ou avoir des séances à l'emploi du temps.") }}</div>
    </div>
@else
<div style="display:flex;flex-direction:column;gap:16px;">
    ...
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:12px 18px;display:flex;align-items:center;gap:10px;color:#92400e;">
        ...
        <div style="font-size:13.5px;">
            <strong>{{ __('Jour férié : :name.', ['name' => $this->holiday->name]) }}</strong>
            {{ __("Normalement il n'y a pas cours ce jour ; vérifiez avant de saisir l'appel.") }}
        </div>
    </div>
    @endif
    ...
    <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:16px 20px;box-shadow:0 1px 3px rgba(16,24,40,.05);display:flex;flex-wrap:wrap;gap:18px;align-items:flex-end;">
        ...
        <div>
            <label style="display:block;font-size:12px;font-weight:600;color:#64748b;margin-bottom:5px;">{{ __('Date') }}</label>
        ...
        <div style="display:flex;gap:8px;">
            <button wire:click="markAll('present')" type="button" style="background:#ecfdf5;color:#059669;border:1px solid #a7f3d0;border-radius:8px;padding:8px 14px;font-size:13px;font-weight:600;cursor:pointer;">{{ __('Tous présents') }}</button>
            <button wire:click="save" type="button" style="background:#2563eb;color:#fff;border:none;border-radius:8px;padding:9px 20px;font-size:13.5px;font-weight:600;cursor:pointer;box-shadow:0 1px 3px rgba(37,99,235,.3);">{{ __("Enregistrer l'appel") }}</button>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;">
        @foreach([[__('Présents'),$s['present'],'#10b981','#ecfdf5'],[__('Absents'),$s['absent'],'#ef4444','#fef2f2'],[__('Retards'),$s['late'],'#f59e0b','#fffbeb'],[__('Excusés'),$s['excused'],'#6366f1','#eef2ff']] as $kpi)
    ...
    <div style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;overflow:hidden;box-shadow:0 1px 3px rgba(16,24,40,.05);">
        ...
        <div style="display:flex;align-items:center;gap:14px;padding:12px 18px;border-bottom:1px solid #f1f4f8;">
            ...
            <div style="display:flex;gap:6px;">
                ...
                <button type="button" wire:click="setStatus({{ $student->id }}, '{{ $opt[0] }}')" title="{{ ['present'=>__('Présent'),'late'=>__('Retard'),'absent'=>__('Absent'),'excused'=>__('Excusé')][$opt[0]] }}"
        ...
        @empty
        <div style="padding:40px;text-align:center;color:#94a3b8;font-size:14px;">{{ __('Aucun élève dans cette classe.') }}</div>
        @endforelse
    </div>

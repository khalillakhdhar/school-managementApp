...
<div id="report-card" style="background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:32px 36px;box-shadow:0 1px 3px rgba(16,24,40,.05);max-width:780px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #0f172a;padding-bottom:16px;margin-bottom:20px;">
        <div>
            ...
            <div style="font-size:12px;color:#64748b;">Bulletin de notes — {{ $report['termLabel'] }}</div>
        </div>
        <div style="text-align:right;">
            <div style="font-size:15px;font-weight:700;color:#0f172a;">{{ $report['student']->full_name }}</div>
            <div style="font-size:12px;color:#64748b;">Classe {{ $report['student']->classroom?->name ?? '—' }}</div>
        </div>
    </div>

    @if(! $report['hasGrades'])
        <div style="padding:30px;text-align:center;color:#94a3b8;font-size:14px;">Aucune note saisie pour ce trimestre.</div>
    @else
    {{-- Tableau des matières --}}
    <table style="width:100%;border-collapse:collapse;margin-bottom:22px;">
        <thead>
            <tr style="background:#fafbfc;">
                <th style="text-align:left;padding:10px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #eaeef3;">Matière</th>
                <th style="text-align:center;padding:10px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #eaeef3;">Note /20</th>
                <th style="text-align:center;padding:10px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #eaeef3;">Coef.</th>
                <th style="text-align:right;padding:10px 14px;font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;border-bottom:1px solid #eaeef3;">Points</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr style="background:#f8fafc;border-top:2px solid #e5e9f0;">
                <td style="padding:11px 14px;font-size:13px;font-weight:700;color:#0f172a;">Total</td>
                <td></td>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">
        <div style="background:linear-gradient(135deg,#2563eb,#1d4ed8);border-radius:12px;padding:16px 18px;color:#fff;">
            <div style="font-size:11px;opacity:.85;font-weight:600;text-transform:uppercase;">Moyenne générale</div>
        ...
        <div style="background:#f8fafc;border:1px solid #e5e9f0;border-radius:12px;padding:16px 18px;">
            <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;">Rang</div>
        ...
        <div style="background:#f8fafc;border:1px solid #e5e9f0;border-radius:12px;padding:16px 18px;">
            <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;">Mention</div>
            <div style="font-size:18px;font-weight:800;margin-top:8px;color:{{ in_array($report['mention'],['Félicitations','Très bien']) ? '#059669' : ($report['mention']==='Insuffisant' ? '#dc2626':'#b45309') }};">{{ $report['mention'] ?? '—' }}</div>
        </div>
    </div>
    @endif
</div>

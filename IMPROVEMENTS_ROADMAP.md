...
11. ✅ **Internationalisation AR/EN complète (RTL)**. Audit exhaustif : **481 chaînes FR→EN** (`lang/en.json`, créé — n'existait pas) et **781 entrées AR** (`lang/ar.json`, complété depuis 174). Tout le texte codé en dur dans `app/Filament` (labels, placeholders, `Section::make()`, `getNavigationLabel/Group`, `getModelLabel`, notifications `->title()/->body()`) enveloppé dans `__()` (444+ remplacements scriptés + correctifs manuels). Les 13 vues Blade portail (staff + parent) entièrement traduites (dashboards, bulletin, emploi du temps, paie, présences). Jours de la semaine (`Lundi`→`Monday`/`الإثنين`) traduits à l'affichage sans toucher aux valeurs FR stockées en base. `now()->locale('fr')` corrigé en `now()->locale(app()->getLocale())` partout. **RTL** : propriétés CSS directionnelles (`border-left/right`, `margin-left/right`, `padding-left/right`, `text-align:left/right`, `position:sticky;left:0`) converties en propriétés logiques (`-inline-start/end`, `inset-inline-start`) sur 17 fichiers (70+ remplacements) ; `->alignRight()` Filament → `->alignEnd()`. Le switch `dir="rtl"` (render hook global, déjà en place) et le sélecteur de langue (`bezhansalleh/filament-language-switch`, auto-enregistré sur tous les panels) fonctionnaient déjà correctement. **13/13 tests verts**, vues Blade compilées sans erreur.

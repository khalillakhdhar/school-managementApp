...
11. ✅ **Internationalisation AR/EN complète (RTL)**. Audit exhaustif : **1067 chaînes FR→EN** (`lang/en.json`) et **1364 entrées AR** (`lang/ar.json`) au terme de 3 correctifs successifs. Tout le texte codé en dur dans `app/Filament` (labels, placeholders, `Section::make()`, `getNavigationLabel/Group`, `getModelLabel`, notifications `->title()/->body()`) enveloppé dans `__()`. Les 13 vues Blade portail (staff + parent) entièrement traduites (dashboards, bulletin, emploi du temps, paie, présences). **Correctif 2 (post-retour utilisateur)** : le contenu *intérieur* des pages (pas seulement la navigation) restait en français quel que soit le locale — corrigé pour les widgets du dashboard admin, les pages admin custom (`class-timetable`, `teacher-schedule`, `financial-report`, `demo-mode`, `report-cards`, `school-settings`). **Correctif 3** : (a) `lang/{fr,ar}/validation.php`+`auth.php`+`passwords.php` créés — bug critique où toute erreur de validation de formulaire affichait la clé brute en FR/AR (mécanisme Laravel natif séparé du système JSON `__()`) ; (b) 4 emails transactionnels, la landing page publique (536 lignes) et la page de changement de mot de passe forcé traduits ; (c) les 13 `*ListStatsWidget.php` + `BlogStatsWidget.php` (totalement oubliés par le scan initial, pattern `Stat::make()` non couvert) + ~20 fichiers `Resources/` (enums de statut, empty states, RelationManager `getTitle()`) traduits ; (d) 9 nouvelles occurrences de `now()->locale('fr')` corrigées en `locale(app()->getLocale())` (`DemoDataService.php` volontairement laissé en FR — données de démo fictives, pas de l'UI). Jours de la semaine (`Lundi`→`Monday`/`الإثنين`) traduits à l'affichage sans toucher aux valeurs FR stockées en base. **RTL** : propriétés CSS directionnelles converties en propriétés logiques (`-inline-start/end`, `inset-inline-start`) ; `->alignRight()` Filament → `->alignEnd()`. Le switch `dir="rtl"` et le sélecteur de langue fonctionnaient déjà correctement. Scans finaux automatisés confirment l'absence de texte français résiduel non traduit (hors placeholders d'exemple volontairement non traduits et données PDF officielles). **13/13 tests verts**, vues Blade compilées sans erreur. *(Hors scope actuel : items 12-13 ci-dessous, exclus explicitement par l'utilisateur.)*
12. **Mode multi-établissement (SaaS / tenancy)** si ouverture à plusieurs écoles.
13. ✅ **Policies explicites par modèle.** 6 Policies existaient déjà (`Student`, `Payment`, `Payroll`, `Grade`, `Incident`, `Employee`, trait commun `Concerns\ChecksSchoolAccess`) mais `PERMISSIONS_AUDIT.md` l'ignorait. 12 Policies manquantes créées (`Level`, `Service`, `Expense`, `ExpenseCategory`, `AuditLog`, `BlogPost`, `Holiday`, `Attendance`, `StudentAttendance`, `Classroom`, `Subject`, `TimetableEntry`) et enregistrées dans `AppServiceProvider::boot()` — **18/18 modèles couverts**. `Resource::skipAuthorization()` conservé volontairement pour l'UI Filament (mono-admin) ; les Policies restent enforced sur l'API REST + les contrôles `abort(403)` des portails. 6 tests PHPUnit ajoutés (`tests/Feature/PolicyTest.php`). Voir `POLICIES_ROADMAP.md` pour le détail.
